...
public class Afficher_CommentaireController implements Initializable {
    ...
    @FXML
    private void GOStat_Comm(ActionEvent event) {
        try {
     
            Parent parent = FXMLLoader.load(getClass().getResource("/GUI/Commentaire_Stats.fxml"));
    ...
    private void Excel(ActionEvent event) throws SQLException, IOException {
         CommentaireService bs= new CommentaireService();
         ObservableList <Commentaire> list;
         list = bs.getCommentaireList();
       Writer writer = null;
        try {
            //badel path fichier excel
            File file = new File("C:\\Users\\zeine\\OneDrive\\Bureau\\feel_the_burn\\Commentaire.csv");
            writer = new BufferedWriter(new FileWriter(file));
            for (Commentaire ev : list) {

                String text = ev.getId_com() +"," +ev.getNom_c()+ "," + ev.getEmail()+ "," + ev.getMessage()+","+ev.getDate()+","+ev.getNom_article()+ ","+ev.getId_b()+ "\n";
