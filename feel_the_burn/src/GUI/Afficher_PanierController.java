...
public void delete()
   ...
   private volatile boolean stop = false;
    ...
    private void Excel(ActionEvent event) throws SQLException, IOException {
        	PanierService bs= new PanierService();
         ObservableList <Panier> list;
         list = bs.getPanierList();
       Writer writer = null;
        try {
            //badel path fichier excel
            File file = new File("C:\\Users\\zeine\\OneDrive\\Bureau\\feel_the_burn\\Panier.csv");
            writer = new BufferedWriter(new FileWriter(file));
            for (Panier ev : list) {

                String text = ev.getId_Pa() +"," +ev.getQuantite()+ "," + ev.getCoupon()+ "," + ev.getId_P()+ "\n";
