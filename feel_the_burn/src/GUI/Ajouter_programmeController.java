...
public class Ajouter_programmeController implements Initializable {
     ...
     private void notiff()
    {
        ...
        Programme p = new Programme();
        int y=ps.calculnb(Integer.valueOf(id.getText()));
        TrayNotification tray = new TrayNotification();
        AnimationType type = AnimationType.POPUP;
        tray.setAnimationType(type);
        tray.setTitle("Programme Valides");
        tray.setMessage("il existe "+y+ " plusieurs programmes");
tray.setNotificationType(NotificationType.INFORMATION);
tray.showAndDismiss(Duration.millis(2000));
    }
    private void notiff(MouseEvent event) {
        notiff();
    }
